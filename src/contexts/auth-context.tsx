...
import { syncUserActivity } from "@/lib/supabase/presence";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Presence Sync: Update last_active every 5 minutes if authenticated
  useEffect(() => {
    if (!isAuthenticated || !user || !role || !isMounted) return;

    // Initial sync
    syncUserActivity(user.id, role);
