...
class AuditLogger {
  ...
  private async persistToDatabase(entry: AuditLogEntry, accessToken?: string): Promise<void> {
    try {
      ...
      // Only attempt getUser if we are missing critical info
      if (!currentUserId || !organizationId || !userRole) {
        try {
          ...
          if (user) {
            ...
            // Final fallback for role from stakeholders table
          ...
          console.warn('[AuditLogger] Auth context check failed, using metadata only');
        }
      }
