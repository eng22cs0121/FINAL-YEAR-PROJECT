...
export async function fetchAnalyticsData() {
  ...
  const { data: anomalies } = await supabase
    .from("anomalies")
    .select("id, batch_id, detected_at, status");
  ...
  anomalies?.forEach((anomaly: any) => {
    const d = new Date(anomaly.detected_at);
...
export async function fetchGlobalShipments() {
  const supabase = await createClient();
  
  // Fetch active batches with their current location/coordinates
  const { data: batches } = await supabase
    .from("batches")
    .select("id, name, status, last_location, latitude, longitude, updated_at")
    .neq("status", "Sold")
    .neq("status", "Rejected");
  ...
  return (batches || []).map(b => ({
    lat: b.latitude || 0,
    lng: b.longitude || 0,
    location: b.last_location || "Unknown",
    status: b.status,
