...
/**
 * Updates the status of an anomaly in Supabase
 */
export async function updateAnomalyStatus(anomalyId: string, status: string, notes?: string, accessToken?: string) {
    const supabase = createClient();
    ...
    if (accessToken) {
        try {
            await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/anomalies?id=eq.${anomalyId}`, {
                method: 'PATCH',
                headers: {
                    'apikey': process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
                    'Authorization': `Bearer ${accessToken}`,
            ...
            console.error("❌ BYPASS Anomaly status update failed:", e);
        }
    }
