...
import { logBatchAction } from '../audit/logger';
...
export async function upsertBatch(batch: Partial<Batch> & { id: string }, userId?: string, accessToken?: string) {
    ...
    if (batch.organization_id) {
        ...
        try {
            ...
            // SECURITY LOGGING: Record the batch creation
            await logBatchAction(
                userId || 'unknown',
                'manufacturer@system.local', // Fallback, usually handled by logger.ts
                'manufacturer',
                batch.id,
                'create',
                true,
                { organization_id: batch.organization_id, accessToken: !!token }
            );
    ...
    if (accessToken) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const updatedBatch = Array.isArray(results) ? results[0] : results;
                
                logDbOperation(originDashboard, `STATUS → ${status} (BYPASS)`, "batches", batchId, true);
                
                // SECURITY LOGGING: Record the status change
                await logBatchAction(
                  userId || 'unknown', 
                  'stakeholder@system.local', 
                  originDashboard.toLowerCase() as any,
                  batchId,
                  'status_change',
                  true,
                  { status, location, txHash, accessToken }
                );
