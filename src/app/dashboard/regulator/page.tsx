...
import { DashboardMap, type MapPosition } from "@/components/dashboard/map";
import { fetchGlobalShipments } from "@/lib/supabase/analytics";
import { Globe } from "lucide-react";
import { UserRole } from "@/lib/blockchain";
import { updateAnomalyStatus } from "@/lib/supabase/alerts";
...
export default function RegulatorDashboard() {
  ...
  const [globalShipments, setGlobalShipments] = useState<MapPosition[]>([]);
  ...
  // Load global shipments for map
  useEffect(() => {
    async function loadShipments() {
      setIsMapLoading(true);
      try {
        const shipments = await fetchGlobalShipments();
        setGlobalShipments(shipments);
      } catch (e) {
        console.error("Failed to load global shipments", e);
      } finally {
  ...
  const handleApproval = async () => {
    ...
    try {
      ...
      if (isFlaggedBatch) {
        ...
        await updateBatchStatus(selectedBatch.id, previousStatus as any, "Regulatory Review Cleared", "Cleared after regulator review");

        // SYNC: Update anomaly status in DB
        try {
          // Find the anomaly ID for this batch if it exists
          const { data: batchAnomalies } = await (await import("@/lib/supabase/client")).createClient()
            .from('anomalies')
            .select('id')
          ...
          if (batchAnomalies && batchAnomalies.length > 0) {
            await updateAnomalyStatus(batchAnomalies[0].id, 'resolved', 'Cleared by regulator during manual review.');
          }
        } catch (e) {
          console.warn("Failed to sync anomaly resolution", e);
        }
        ...
        <Tabs defaultValue="pending" className="space-y-4">
          ...
          <TabsContent value="tracking">
            <Card className="glass-panel dark:glass-panel-dark">
              <CardHeader>
                <CardTitle>Global Supply Chain Fleet</CardTitle>
                <CardDescription>Real-time location tracking of all active medicine batches globally.</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-[600px] w-full rounded-lg overflow-hidden border border-border relative">
                  {isMapLoading ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted/30 text-center p-6">
                      ...
                      <h4 className="text-lg font-semibold">No Active Shipments</h4>
                      <p className="text-muted-foreground max-w-xs">There are currently no active batches in transit to display on the global map.</p>
                    </div>
                  ) : null}
                  <DashboardMap positions={globalShipments} showRoute={false} height="600px" />
                </div>
              </CardContent>
            </Card>
          </TabsContent>
